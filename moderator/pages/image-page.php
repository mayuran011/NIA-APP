<?php
/**
 * Admin: Image page builder (layout for /images).
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save_boxes') {
        ...
        $boxes = [];
        foreach ($raw as $b) {
            $type = isset($b['type']) ? trim($b['type']) : 'image';
            ...
            $channel_type = isset($b['channel_type']) ? trim($b['channel_type']) : 'image';
        ...
        redirect(admin_url('image-page'));
    }
}

$admin_title = 'Image page builder';
...
<div class="card shadow-sm border-0 mb-4">
    <div class="card-header bg-white border-bottom d-flex justify-content-between align-items-center py-3">
        <h5 class="mb-0 fw-bold d-flex align-items-center">
            <span class="material-icons text-primary me-2">image</span> Image Page Builder
        </h5>
        <a href="<?php echo url('images'); ?>" class="btn btn-sm btn-light border d-inline-flex align-items-center shadow-sm" target="_blank">
            <span class="material-icons me-1" style="font-size: 16px;">open_in_new</span> View /images
        </a>
    </div>
    <div class="card-body bg-light">
        <p class="text-muted small mb-4 d-flex align-items-center">
            ...
            Design sections for your Images page (galleries, latest images, featured channels, etc.). Drag cards to reorder.
        </p>
        <form method="post">
            ...
            <div id="homepage-sections" class="mb-4 d-flex flex-column gap-3">
                <?php foreach ($boxes as $i => $box) {
                    $type = isset($box['type']) ? $box['type'] : 'image';
                    ...
                    $channel_type = isset($box['channel_type']) ? $box['channel_type'] : 'image';
                        ...
                        <div class="d-flex align-items-center gap-3">
                            ...
                            <div class="flex-grow-1 row g-3 align-items-end">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <input type="text" class="form-control border-secondary border-opacity-25" name="boxes[<?php echo $i; ?>][title]" value="<?php echo _e($title); ?>" placeholder="e.g. Latest Images">
                                </div>
                                ...
                                <div class="col-md-3 section-channel-type-wrap">
                                    ...
                                    <select class="form-select border-secondary border-opacity-25" name="boxes[<?php echo $i; ?>][channel_type]">
                                        <option value="image" <?php echo ($channel_type ?? '') === 'image' ? 'selected' : ''; ?>>Image channels</option>
                                        <option value="video" <?php echo ($channel_type ?? '') === 'video' ? 'selected' : ''; ?>>Video channels</option>
                                        <option value="music" <?php echo ($channel_type ?? '') === 'music' ? 'selected' : ''; ?>>Music channels</option>
                                    </select>
                                </div>
...
<?php
// Reuse the same template + JS behaviour from video-page builder
